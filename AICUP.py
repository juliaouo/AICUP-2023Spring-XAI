...
def get_pred_pages(series_data: pd.Series) -> Set[Dict[int, str]]:
    ...
    if len(results) > DOC_RE_NUM:
        assert -1 not in mapping.values()
        if len(list(mapping)) > DOC_RE_NUM:
            results = sorted(random.sample(list(mapping), DOC_RE_NUM))
        else:
            results = sorted(mapping, key=mapping.get)
        # results = sorted(mapping, key=mapping.get)[:DOC_RE_NUM]
...
# %%
doc_path = f"data/train_doc{DOC_RE_NUM}.jsonl"
...
# %%
test_doc_path = f"data/test_doc{DOC_RE_NUM}.jsonl"
...
TRAIN_DATA = load_json("data/public_train.jsonl")
TEST_DATA = load_json("data/public_test.jsonl")
DOC_DATA = load_json(f"data/train_doc{DOC_RE_NUM}.jsonl")
...
# %%
EXP_DIR = f"sent_retrieval/e{NUM_EPOCHS}_bs{TRAIN_BATCH_SIZE}_" + f"{LR}_neg{NEGATIVE_RATIO}_top{SEN_RE_NUM}"
...
# %%
ckpt_name = "model.150.pt"  #@param {type:"string"}
...
train_results = evaluate_retrieval(
    ...
    top_n=SEN_RE_NUM,
    save_name=f"train_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.jsonl",
)
...
val_results = evaluate_retrieval(
    ...
    ground_truths=DEV_GT,
    top_n=SEN_RE_NUM,
    save_name=f"dev_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.jsonl",
)
...
# %%
test_data = load_json(f"data/test_doc{DOC_RE_NUM}.jsonl")
...
evaluate_retrieval(
    ...
    top_n=SEN_RE_NUM,
    cal_scores=False,
    save_name=f"test_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.jsonl",
)
...
TRAIN_DATA = load_json(f"data/train_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.jsonl")
DEV_DATA = load_json(f"data/dev_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.jsonl")

TRAIN_PKL_FILE = Path(f"data/train_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.pkl")
DEV_PKL_FILE = Path(f"data/dev_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.pkl")
...
# %%
OUTPUT_FILENAME = f"submission_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.jsonl"

EXP_DIR = f"claim_verification/e{NUM_EPOCHS}_bs{TRAIN_BATCH_SIZE}_" + f"{LR}_top{SEN_RE_NUM}"
LOG_DIR = "logs/" + EXP_DIR
CKPT_DIR = "checkpoints/" + EXP_DIR
...
# %%
TEST_DATA = load_json(f"data/test_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.jsonl")
TEST_PKL_FILE = Path(f"data/test_doc{DOC_RE_NUM}sent{SEN_RE_NUM}.pkl")
...
# %%
ckpt_name = "val_acc=0.4370_model.1725.pt"  #@param {type:"string"}
model = load_model(model, ckpt_name, CKPT_DIR)
